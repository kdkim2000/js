...
| `data/registry.json` | 등록 사이트 목록 (크롤러가 자동 생성) |
| `crawler/generic/` | 제너릭 크롤러 (메인), CommonJS |
| `crawler/utils/` | 공통 유틸 (html-to-md, rate-limiter) |
...
- `User-Agent` 헤더 반드시 명시: `MultiSite-Crawler/1.0`
- 타임아웃 **15,000ms** 이상 설정
- 실패 시 지수 백오프 재시도 (최대 3회)

### 파일 저장 규칙

- 아티클 파일명: `{slug}.md` (슬러그는 URL 경로 기반)
- 이미지 경로: `data/sites/{siteId}/images/{slug}/{filename}`
- 기존 파일 존재 시 스킵 (덮어쓰기 금지)
- 실패 아티클은 `data/sites/{siteId}/errors.json`에 누적 기록
...
| 규칙 | 내용 |
|------|------|
| `output: 'export'` 조건부 적용 | `NEXT_EXPORT=1`일 때만 (`next.config.ts`) |
| 이미지 최적화 비활성화 | `images: { unoptimized: true }` |
| API Routes `force-static` 선언 | 빌드 요구사항; POST 포함 라우트는 ƒ Dynamic으로 처리됨 |
| 검색 API | GET: 빌드용 빈 응답; POST: 실제 SQLite 검색 |
...
- 반환 형식: `NextResponse.json(data)`.
- 에러는 빈 배열/객체 반환 (500 에러 대신 graceful fallback).
- 모든 API Route에 `export const dynamic = 'force-static'` 선언 (`output: 'export'` 빌드 요구사항).
- 검색처럼 query params가 필요한 GET 라우트는 POST body 방식으로 전환 (force-static이 GET searchParams를 null로 처리함).
...
```
feat: [크롤러/사이트/mcp] 기능 설명
fix: [크롤러/사이트/mcp] 버그 설명
docs: 문서 업데이트
refactor: 코드 정리
```
...
```ts
// ❌ siteId 하드코딩
const dir = getSiteDataDir('ko-javascript-info');

// ✅ 상수 사용
import { DEFAULT_SITE_ID } from '@/lib/registry';
const dir = getSiteDataDir(DEFAULT_SITE_ID);
```
...
| 검색 응답 (개발) | < 100ms | SQLite FTS5 인덱스 |
